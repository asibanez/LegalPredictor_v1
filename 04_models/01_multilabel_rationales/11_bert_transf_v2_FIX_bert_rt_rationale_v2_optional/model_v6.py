...
# v6 -> Ads logit computation before Gumbel layer
#       Removes for loop in generator for Gumbel computation
...
class ECHR2_model(nn.Module):
    ...
    def forward(self, X_facts_ids, X_facts_token_types, X_facts_attn_masks, args):
...
#                          'token_type_ids': facts_token_types.long(),
        ...
        if eval(args.rationales) == True:
            ...
            logits =  F.log_softmax(x_Q, dim = -1)                          # batch_size x max_n_pars x 2
            mask = F.gumbel_softmax(logits, tau = self.gumbel_temp,
