...
# v1 -> Uses two different enconders for arts and facts
...
class ECHR2_model(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.bert_model_echr = AutoModel.from_pretrained(self.model_name)
        self.bert_model_facts = AutoModel.from_pretrained(self.model_name)
    ...
    def BERT_enc_echr_f(self, X_ids, X_token_types, X_attn_masks,
        ...
        for idx in range(0, max_num_pars):
            ...
            # Compute bert output
            output = self.bert_model_echr(**bert_input,
    ...
    def BERT_enc_facts_f(self, X_ids, X_token_types, X_attn_masks,
                      max_num_pars, batch_size, empty_par_ids, device):
        ...
        for idx in range(0, max_num_pars):
            ...
            # Compute bert output
            output = self.bert_model_facts(**bert_input,
                                           output_hidden_states = True) # Tuple
            bert_out[idx] = output['pooler_output'].unsqueeze(1)        # batch_size x 1 x h_dim
    ...
    def forward(self, X_facts_ids, X_facts_token_types, X_facts_attn_masks,
        ...
        x_facts, transf_mask_facts = self.BERT_enc_facts_f(X_facts_ids,
                                                           X_facts_token_types,
                                                           X_facts_attn_masks,
        ...
        x_echr, transf_mask_echr = self.BERT_enc_echr_f(X_echr_ids,
                                                        X_echr_token_types,
                                                        X_echr_attn_masks,
