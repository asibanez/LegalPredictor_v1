...
# v2 -> ECHR artiles removed. Uses only facts for binary prediction
...
#                                out_features = self.n_labels)
       ...
       self.fc_out = nn.Linear(in_features = self.max_n_pars_facts*self.h_dim,
                               out_features = self.n_labels)
        ...
        # Batch normalization
        self.bn1 = nn.BatchNorm1d(self.max_n_pars_facts*self.h_dim)
...
#        x = torch.cat([x_facts, x_echr], dim = 1)                       # batch_size x (max_n_pars_facts + max_n_pars_echr) x h_dim
        x = x_facts                                                     # batch_size x max_n_pars_facts x h_dim
...
#                          self.max_n_pars_echr)*self.h_dim)             # batch_size x ((max_n_pars_facts + max_n_pars_echr) x h_dim)
        ...
        x = x.reshape(-1, self.max_n_pars_facts*self.h_dim)             # batch_size x (max_n_pars_facts x h_dim)
            
#        x = self.bn1(x)                                                 # batch_size x ((max_n_pars_facts + max_n_pars_echr) x h_dim)
        x = self.bn1(x)                                                 # batch_size x (max_n_pars_facts x h_dim)       
